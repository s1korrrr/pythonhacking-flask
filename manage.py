...
def fill_db(conn: SQLAlchemy):
    ...
    conn.session.add(Owner(name='Patryk', password='admin', email='pat@mail.pl'))
    conn.session.add(Owner(name='Nikodem', password='admin', email='nik@mail.pl'))
    conn.session.add(Owner(name='Andrzej', password='admin', email='and@mail.pl'))
